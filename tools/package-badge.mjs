...
const root = new URL('../firmware/badge/build/', import.meta.url);
const merged = await readFile(new URL('merged-binary.bin', root));
const app = await readFile(new URL('folotoy-identity-badge.bin', root));
...
assert.deepEqual(partitions.get('badge'), [0x35a000, 0x3a6000]);
const output = new URL('../public/assets/firmware/identity-badge.bin', import.meta.url);
await writeFile(output, merged);
const release = new URL('../artifacts/identity-badge/', import.meta.url); await mkdir(release, { recursive: true });
for (const [source, target] of [['bootloader/bootloader.bin','bootloader.bin'], ['partition_table/partition-table.bin','partition-table.bin'], ['folotoy-identity-badge.bin','application.bin'], ['merged-binary.bin','identity-badge.bin']]) await copyFile(new URL(source, root), new URL(target, release));
await copyFile(new URL('../firmware/badge/README.zh_CN.md', import.meta.url), new URL('README.zh_CN.md', release));
await copyFile(new URL('../firmware/badge/README.md', import.meta.url), new URL('README.md', release));
await copyFile(new URL('../firmware/badge/components/bsp/LICENSE', import.meta.url), new URL('LICENSE', release));
...
console.log(`Firmware PASS: ${app.length} byte app, ${merged.length} byte merged image; partition MD5, protected layout and image contents verified. Packaged in artifacts/identity-badge/.`);
